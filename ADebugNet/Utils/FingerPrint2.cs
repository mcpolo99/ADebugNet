...
namespace ADebugNet.Utils
{
    internal class FingerPrint2
    {
        ...
        //just to detarmine that machine is virtual or not..
        internal static bool IsVirtualMachine
        {
            get
            {
                bool isVirtualMachine = false; //default is false
                try
                {
                    string str = ComputerSystemModel();
                    if (!string.IsNullOrWhiteSpace(str))
                        isVirtualMachine = (" " + str.ToLower() + " ").Replace("box", string.Empty).Contains(" virtual "); //returns a true if computer includes virtual
                }
                catch (Exception ex)
                {
                }
        ...
        //gets a hash from fingerprint Value() Can be used as a extra controll mechanism
        ...
        public static string Value()
        {
            ...
            if (string.IsNullOrWhiteSpace(fingerPrint)) //if fingerprint is empty execute
            {
                bool isVirtualMachine = IsVirtualMachine; //get Virtualmachine
                try // inside here will be Fingerprint if virtualmachine
                {
                    str = BaseId();
                ...
                if (!isVirtualMachine) // Checks if Virtual machine, if its false override fingerprint with new string  
                {
